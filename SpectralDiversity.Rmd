...
Import_TC <- function(tc_list){
  tc_df = tibble()
  for(i in tc_list){
      indv_tc_df <- read_csv(i, na=c("","NA")) %>% select(c('PrimaryKey', contains('Histogram')))
...
tc_hists <- tc_hists_all %>% select(-PrimaryKey)
...
```{r}
species_div <- read_csv('data/SpectralDiversity_CSVs/Allmodel.csv') %>%
  select(c(PrimaryKey,SpeciesN))
```
...
spectral_mns <- read_csv('data/SpectralDiversity_CSVs/RS_Predictors.csv') %>% 
  drop_na() %>% 
  select(c(PrimaryKey, contains('_mn')))
all_div_mns <- left_join(all_div, spectral_mns, by = 'PrimaryKey') %>% select(-PrimaryKey)

# Remove PrimaryKey column from all_div
all_div <- all_div %>% select(-PrimaryKey)
